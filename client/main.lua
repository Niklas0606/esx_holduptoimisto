...
RegisterNetEvent('esx_holduptoimisto:currentlyRobbing')
AddEventHandler('esx_holduptoimisto:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_holduptoimisto:killBlip')
AddEventHandler('esx_holduptoimisto:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_holduptoimisto:setBlip')
AddEventHandler('esx_holduptoimisto:setBlip', function(position)
...
RegisterNetEvent('esx_holduptoimisto:tooFar')
AddEventHandler('esx_holduptoimisto:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_holduptoimisto:robberyComplete')
AddEventHandler('esx_holduptoimisto:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_holduptoimisto:startTimer')
AddEventHandler('esx_holduptoimisto:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_holduptoimisto:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_holduptoimisto:tooFar', store)
			end
		end
	end
end)
